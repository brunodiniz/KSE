...
//    }
    ...
    public KuabaGraphOld(KuabaRepository source) {
        this.source = source;
    }
    ...
    private void initializeGraph() {
        ...
        try {
            ...
            Logger.getLogger(KuabaGraphOld.class.getName()).log(Level.SEVERE, null, ex);
...
//            if (decisionMaking) graph.getViewPane().addMouseListener(new DecisionMakerPopup(graph));
            ...
            graph.setMaximizable(false);
            ((javax.swing.plaf.basic.BasicInternalFrameUI) graph.getUI()).setNorthPane(null);
    ...
    private void changeSelectedNodes(String linkType) {
            ...
            while (nodeEnum.hasMoreElements()) {
                ...
                for(int x = 0; x<linksForNode.size(); x++) {
                    ...
                    if (l.getId().contains(KuabaGraphOld.IS_ADDRESSED_BY_ID_COMPONENT)) try {
        ...
        private void acceptQuestion(NodeSummary question) {
            if(question.getLabel().contains(Question.DOMAIN_QUESTION_TEXT_PREFIX)) {
                ...
                for(int y = 0; y<questionLinks.size(); y++) {
                    ...
                    if (lnk.getLabel().equals(KuabaGraphOld.SUGGESTS_LABEL) && question.getLabel().contains(lnk.getFrom().getLabel()))
        ...
        private void rejectIdea(NodeSummary idea) {
            Vector<Link> linksForNode = map.getView().getLinksForNode(idea.getId());
                ...
                for(int x = 0; x<linksForNode.size(); x++) {
                    ...
                    if (l.getId().contains(KuabaGraphOld.IS_ADDRESSED_BY_ID_COMPONENT)) try {
                        if (l.getFrom().getId().equals(Question.ROOT_QUESTION_ID) || idea.getLabel().equals("Association") || idea.getLabel().equals("AssociationEnd") || idea.getLabel().equals("Attribute")) {
                            l.setType(ICoreConstants.OBJECTS_TO_LINK);
                                for(int y = 0; y<linksForNode.size(); y++) {
                                    Link l2 = linksForNode.get(y);

                                    if (l2.getId().contains(KuabaGraphOld.SUGGESTS_ID_COMPONENT)) 
...
//            if(question.getLabel().contains(Question.DOMAIN_QUESTION_TEXT_PREFIX)) {
                ...
                for(int y = 0; y<questionLinks.size(); y++) {
                    ...
                    if (lnk.getId().contains(KuabaGraphOld.IS_ADDRESSED_BY_ID_COMPONENT)) try {
                        lnk.setType(ICoreConstants.OBJECTS_TO_LINK);
                        if(!question.getLabel().equals("participant?") && !question.getLabel().equals("owner?")) rejectIdea(lnk.getTo()); //avoid to reject the participants when rejecting an association
                    } catch (Exception ex) {
                        Logger.getLogger(KuabaGraphOld.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
